...
def apply_black_mask_and_save(script_directory, mask_areas, new_folder):
    # 新しいフォルダを作成（存在しない場合）
    ...
    png_files = glob.glob(script_directory + '/**/*.png', recursive=True)
    ...
    for file_path in png_files:
        ...
        if new_folder in file_path:
            ...
            for mask_area in mask_areas:
                cv2.rectangle(image, mask_area[0], mask_area[1], (0, 0, 0), -1)
            print(image.shape)
...
# 新しいフォルダ名
new_folder = 'masked_images'

# 関数を実行
apply_black_mask_and_save(script_directory, mask_areas, new_folder)